...
const projectImage = defineModel<imageInterface[] | null>("projectImage", { required: true });
...
<template>
  <div class="space-y-4">
    ...
    <div class="flex gap-4">
      ...
      <Select label="Area unit" :data="metaStore.areaSelectData" v-model="chosenAreaValue" class="flex-1" />
      <div class="flex-1">
        <Select label="Float number separator" :data="metaStore.separatorData" v-model="chosenSeparatorValue" />
        <div class="text-xs opacity-80">
          This config will applay on
          <div class="badge !mx-0 !mb-1 !mt-2 inline-block !py-0">area m2</div>
          and
          <div class="badge !mx-0 !my-0 inline-block !py-0">rooms count</div>
        </div>
      </div>

      <div class="flex-1">
        <Select label="Price number separator" :data="metaStore.separatorData" v-model="chosenPriceSeparatorValue" />
      </div>
    </div>

    <div class="space-y-2">
      <Checkbox v-model="isPriceRoundedValue" title="Rounded Price" class="!w-fit" />

      <div v-if="!is_premium" class="border-b text-lg font-semibold">PREMIUM license</div>

      <Checkbox v-model="removeWatermarkValue" title="Remove watermark" :disabled="!is_premium" class="!w-fit" />

      <div v-if="!is_gold" class="border-b text-lg font-semibold">GOLD license</div>

      <div class="flex w-fit justify-between gap-2">
        <Checkbox v-model="shareableLinkValue" title="Create a shareable link" :disabled="!is_gold" class="w-fit" />

        <div
          class="flex cursor-pointer justify-end"
          @mouseenter="exampleImage = PLUGIN_ASSETS_PATH + 'sharable-link.webp'"
      ...
      <div class="flex w-fit justify-between gap-2">
        ...
        <div
          class="flex cursor-pointer justify-end"
          @mouseenter="exampleImage = PLUGIN_ASSETS_PATH + 'requestCallback.webp'"
          @mouseleave="exampleImage = ''"
        >
          <Info />
        </div>
      ...
      <div v-if="requestCallbackValue" class="flex w-fit justify-between gap-2">
        <Checkbox
        ...
        <div
          class="flex cursor-pointer justify-end"
          @mouseenter="exampleImage = PLUGIN_ASSETS_PATH + 'redirectToCallbackUrl.webp'"
          @mouseleave="exampleImage = ''"
        >
          <Info />
        </div>
      ...
      <p>
        ...
        <span class="cursor-pointer font-semibold text-blue-700 underline" @click="pushToFormResponsesPage"> here</span>
      </p>

      <div v-if="!is360FlowDisabled" class="mt-3 rounded-xl border border-gray-200 bg-gray-50/60 px-4 py-3">
        <div class="flex items-center justify-between gap-4">
          <div class="space-y-1">
            <p class="text-sm font-semibold text-gray-900">360 view mode</p>
            <p class="min-w-max text-xs text-gray-400">Use 360 images feature</p>
          </div>

          <Toggle v-model="is_360_flow" @change="onToggle360FlowChange" />
        </div>

        <Checkbox v-model="pathsHoverFillValue" title="SVG Path visible only on hover" class="mt-4 w-fit" />
      </div>
      ...
      >
        ...
        <p class="mb-1 text-sm font-semibold text-gray-900">360° View Mode requires the 360 Add-on.</p>
        <p class="mb-4 text-xs text-gray-500">
          Activate the 360 Add-on to enable interactive 360° building views with SVG polygon overlays.
        </p>
        <div class="mb-4 cursor-pointer text-blue-400 hover:underline" @click="pushToAddonsPage">Activate</div>
