...
const updateFloor = async () => {
  ...
  const params: any = {
    ...
    floor_image: floor_image.value?.[0]?.id || floorStore.activeFloor?.floor_image?.[0]?.id,
...
const sedDefaultValues = (source: FloorItem) => {
  ...
  conf.value = DEFAULT_CONFIG.find((item) => item.value === source.conf) || { title: "choose", value: "" };
...
<template>
  <div class="mt-14 flex gap-5">
    ...
    <div class="flex flex-col gap-10">
      <form class="sticky top-14 h-fit w-60 rounded-md border border-gray-200 shadow-sm" @submit.prevent="submitForm">
        ...
        <div class="flex flex-col items-center gap-3 p-3">
          ...
          <Input v-model="floor_number" type="number" placeholder="2" label="floor number" required />

          <Select v-model="block" :data="blockSelectData" label="select block" clearable />

          <Select v-model="conf" :data="DEFAULT_CONFIG" label="Status" clearable />
