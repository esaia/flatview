            ...
            Tabs::make()->tabs([
                ...
                Tab::make('Pricing')->schema([
                    Grid::make(3)->schema([
                        ...
                        TextInput::make('offer_price')
                            ...
                            ->label('Offer Price')
                            ->nullable(),
                        Toggle::make('request_price')
                            ->label('Request Price Instead'),
                    ]),
                ]),
                ...
                Tab::make('Click Action')->schema([
                    Select::make('click_action')
                        ...
                        ->nullable(),
                    Grid::make(2)->schema([
                        TextInput::make('follow_link.link')
                            ->label('Link URL')
                            ->url()
                            ->nullable(),
        ...
        return $table
            ...
            ->bulkActions([
                ...
                BulkAction::make('mark_available')
                    ...
                    ->color('success')
                    ->action(fn (Collection $records) => $records->each->update(['conf' => 'available']))
                    ->requiresConfirmation(),
                BulkAction::make('mark_reserved')
                    ->label('Mark as Reserved')
                    ->icon('heroicon-o-clock')
                    ->color('warning')
                    ->action(fn (Collection $records) => $records->each->update(['conf' => 'reserved']))
                    ->requiresConfirmation(),
                BulkAction::make('mark_sold')
                    ->label('Mark as Sold')
                    ->icon('heroicon-o-x-circle')
                    ->color('danger')
                    ->action(fn (Collection $records) => $records->each->update(['conf' => 'sold']))
                    ->requiresConfirmation(),
            ]);
    }
}
