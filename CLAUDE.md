...
- **Media library picker** (`app/Filament/Support/MediaLibrary.php`) is the "Choose from library" hint action attached to Filament `FileUpload` fields. It reads the FileUpload's `->directory(...)` and **opens on that folder** (e.g. `services-gallery`), with a folder select for browsing any other folder on the public disk, or all of them at once. **Rule: any admin upload component using `MediaLibrary::pickerAction()` must set a `->directory()`** — it decides both where uploads land and which folder the picker opens on; without it the picker opens on the whole disk.
- **Environment sync** (`app/Support/SiteSync/`, admin page *Configuration → Export / Import*) packs selected tables and their `public`-disk upload folders into one `.zip` and restores it elsewhere, preserving IDs. It is deliberately **admin-UI only — no artisan commands**. Because Livewire cannot chunk uploads, an export larger than the target's `upload_max_filesize` is split into byte-slice parts (`…zip.001`, …) by `ArchiveParts` and rejoined at import. **Rule: `SyncManifest` is the single source of truth for what syncs** — a new content table or upload directory must be added to a group there (and to `TABLE_ORDER`, parents before children) or it will silently not travel between local and prod. Archives live in `storage/app/private/sync`.
